...
void editor_init(Editor* ed, int argc, char* argv[]) {
    ...
    if (ed->filename) {
        ...
        snprintf(recovery_path, sizeof(recovery_path), "%s.recovery", ed->filename);
...
void auto_save(Editor* ed) {
    if (ed->filename) {
        ...
        snprintf(backup_path, sizeof(backup_path), "%s.bak.%d", ed->filename, ed->backup_count % 10 + 1);
...
void editor_draw(WINDOW* win, Editor* ed) {
    ...
    draw_update(win, &ed->model, &ed->scroll_row, &ed->scroll_col, ed->cursor_line, ed->cursor_col, ed->show_line_numbers, ed->syntax_highlight, ed->search_mode, ed->search_buffer, ed->selection_start_line, ed->selection_start_col, ed->selection_end_line, ed->selection_end_col, ed->selection_active, &dummy_y, &dummy_x, ed->replace_step, ed->replace_buffer, &ed->config, ed);
}
...
void editor_handle_input(Editor* ed, int ch) {
    ...
    if (ch == 18) { // Ctrl+R replace
        ...
        if ((ed->unsaved_keystrokes >= ed->auto_save_threshold ||
             (ed->auto_save_timeout > 0 && now - ed->last_save_time >= ed->auto_save_timeout)) &&
            ed->filename) {
            auto_save(ed);
        }
